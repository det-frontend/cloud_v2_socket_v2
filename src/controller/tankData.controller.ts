...
import { addTankDataService, deleteTankDataById, getAllTankDataService, latestTankDataByStationId, tankDataByDate, updateTankDataService } from "../service/tankData.service";
import fMsg, { fuelBalanceCalculationForStockBalance, realTankCalculationForStockBalance } from "../utils/helper";
import { fuelBalanceForStockBalance } from "../service/fuelBalance.service";
import moment from "moment-timezone";
import { addStockBalanceService, findByoneAndUpdateMany, findStockBalanceByDateService } from "../service/stockBalance.service";
...
export const addTankDataController = async (req: Request, res: Response, next: NextFunction) => {
    ...
    //find fuel balance
    let fuelBalanceLatest = await fuelBalanceForStockBalance(currentDate, stationId, model);
    
    //find yesterday Tank
    let yesterdayTank = await latestTankDataByStationId({stationDetailId:req.body.stationDetailId,dailyReportDate:previousDate},model);
           
    console.log(yesterdayTank.length);
  ...
  //yesterday tank opening
    const yesterdayTankData = await realTankCalculationForStockBalance(yesterdayDatas);
  // yesterday tank opening
  

 //fuel balance opening
    const { ron92_opening, ron92_cash, ron92_balance, ron92_receive,ron95_opening, ron95_cash, ron95_balance,ron95_receive, diesel_opening, diesel_cash, diesel_balance,diesel_receive, pDiesel_opening, pDiesel_cash, pDiesel_balance,pDiesel_receive } = fuelBalanceCalculationForStockBalance(fuelBalanceLatest);
        ...
        const pureData92 = {
            ...
            todayGL: (ron92_cash - (yesterdayTankData.ron92 - todayTankData.ron92)).toFixed(3),
            totalGL: 0,
            accessDb: "kyaw_san",
        ...
        const pureData95 = {
            ...
            todayGL: (ron95_cash - (yesterdayTankData.ron95 - todayTankData.ron95)).toFixed(3),
            totalGL: 0,
            accessDb: "kyaw_san",
            realTime: currentDate
        };
        const pureDiesel = {
            ...
            todayGL: (diesel_cash - (yesterdayTankData.diesel - todayTankData.diesel)).toFixed(3),
            totalGL: 0,
            accessDb: "kyaw_san",
            realTime:currentDate
        };
        const pureSuperDiesel = {
            ...
            todayGL: (pDiesel_cash - (yesterdayTankData.pDiesel - todayTankData.pDiesel)).toFixed(3),
            totalGL: 0,
            accessDb: "kyaw_san",
            realTime:currentDate
        };
...
const isToday = await findStockBalanceByDateService({ realTime: currentDate }, model);
    
        
 if (isToday.length === 0) {
    const dataWeMustSave = [pureData92, pureData95, pureDiesel, pureSuperDiesel];
...
fMsg(res, "Tank data add is successful!", result);
    ...
    const result_1 = await findByoneAndUpdateMany({ tank: "001-Octane Ron(92)",realTime:currentDate }, pureData92,model);
    if (!result_1) return next(new Error(result_1));

    const result_2 = await findByoneAndUpdateMany({ tank: "002-Octane Ron(95)",realTime:currentDate }, pureData95, model);
    if (!result_2) return next(new Error(result_2));

    const result_3 = await findByoneAndUpdateMany({ tank: "005-Premium Diesel",realTime:currentDate }, pureSuperDiesel, model);
     if (!result_3) return next(new Error(result_3));

    const result_4 = await findByoneAndUpdateMany({ tank: "004-Diesel",realTime:currentDate }, pureDiesel, model);
