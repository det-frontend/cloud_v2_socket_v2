...
export const statementReportHandler = async (
  ...
  try {
    ...
    for (let i: number = 1; i <= stationDetail[0].nozzleCount; i++) {
      ...
      let result:any = await detailSaleByDate(query, startDate, endDate, model);
...
try {
  ...
  const stationDetailIds = [
    ...
    new ObjectId(stationIds[6]),
    new ObjectId(stationIds[7]),
    new ObjectId(stationIds[8]),
  ];


    const vehicleQuery = { createAt: { $gt: startDate, $lt: endDate }, stationDetailId: { $in: stationDetailIds } }
